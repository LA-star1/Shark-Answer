"""OpenAI-compatible provider for DeepSeek, Qwen, Grok, MiniMax, Kimi.

These services expose an OpenAI-compatible chat completions API, so we
share a single implementation with configurable base_url and model name.
"""
    ...
    async def generate_with_image(self, prompt: str, image_data: bytes,
        ...
        if not self.supports_vision:
            return ModelResponse(
                ...
                model_name=self.model_name,
                success=False,
                error=f"{self.provider_name} does not support vision",
            )
        return await self._safe_generate(
